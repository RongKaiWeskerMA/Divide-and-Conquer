...
class SyntheticNerfDataset(BaseDataset):
    def __init__(self,
        ...
        # if self.overlap:
        # datadir = os.path.join(datadir, f"{split}_json_{}")
        datadir = os.path.join(datadir, f"{split}_json_4") # uncomment if training synthetic data
        # else:
        #     datadir = os.path.join(datadir, f"{split}_json_")
        ...
        super().__init__(
            datadir=datadir,
            split=split,
            scene_bbox=get_360_bbox(datadir, is_contracted=False), # is_concentrate=False
            is_ndc=False,
            is_contracted=False,
    ...
    def __getitem__(self, index):
        ...
        out["near_fars"] = torch.tensor([[2.0, 6.0]]) # [2.0, 6.0]
        return out
...
def load_360_frames_custom(data_dir, exp_idx, split, max_frames: int) -> Tuple[Any, Any]:
    # json_path = os.path.join(data_dir, f"transforms_{split}_120_{exp_idx}.json")
...
class SyntheticNerfDataset_Distil(BaseDataset):
    def __init__(self,
                 datadir,
                 split: str,
                 batch_size: Optional[int] = None,
        ...
        self.exp_idx = exp_idx
        self.overlap = overlap
        # datadir = os.path.join(datadir, f"{split}_json")
        datadir = os.path.join(datadir, f"{split}_json_4")
        if split == 'render':
            frames, transform = load_360_frames(datadir, 'test', self.max_frames)
            imgs, poses = load_360_images(frames, datadir, 'test', self.downsample)
            render_poses = generate_hemispherical_orbit(poses, n_frames=120)
            ...
            intrinsics = load_360_intrinsics(
                ...
                downsample=self.downsample)
        rays_o, rays_d, imgs = create_360_rays(
            imgs, poses, merge_all=True, intrinsics=intrinsics)
