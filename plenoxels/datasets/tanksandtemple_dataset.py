...
class TanksandTemple(BaseDataset):
    def __init__(self,
                 datadir,
                 split: str,
                 ...
                 exp_idx: Optional[str] = None,
                #  use_splitdir: Optional[bool] = False
                 ):
        ...
        if self.exp_idx:
            datadir = os.path.join(datadir, f"{split}_json") # uncomment if training synthetic data
        # else:
        #     datadir = os.path.join(datadir, "wendy_json")
        if split == 'render':
            ...
            frames, transform = load_360_frames_custom(datadir, self.exp_idx, split, self.max_frames)
        ...
        super().__init__(
            datadir=datadir,
            split=split,
            scene_bbox=get_360_bbox(datadir, is_contracted=True), # is_concentrate=False
            # scene_bbox=torch.from_numpy(np.loadtxt(f'{datadir}/bbox.txt')).float()[:6].view(2,3)*1.2,
            is_ndc=False,
            is_contracted=True,
    ...
    def __getitem__(self, index):
        ...
        # Alpha compositing
        if pixels is not None and pixels.shape[-1] > 3 :
            pixels = pixels[:, :3] * pixels[:, 3:] + bg_color * (1.0 - pixels[:, 3:])
    
        out["imgs"] = pixels
        out["bg_color"] = bg_color
        out["near_fars"] = torch.tensor([[0., 6.]]) # [2.0, 6.0]
        return out
...
# self-implemented loadfrom train/test_json directory
def load_360_frames_custom(data_dir, exp_idx, split, max_frames: int) -> Tuple[Any, Any]:
    json_path = os.path.join(data_dir, f"transforms_{split}_{exp_idx}.json")
...
def load_360_images(frames, datadir, split, downsample) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    imgs, poses, f_name = zip(*img_poses)
    imgs = torch.stack(imgs, 0)  # [N, H, W, 3/4]
    poses = torch.stack(poses, 0)  # [N, ????]
    return imgs, poses

def save_fname(f_name, datadir):
    import pickle
    with open(os.path.join(datadir, 'fnames.pickle'), 'wb') as fp:
        pickle.dump(f_name, fp)
