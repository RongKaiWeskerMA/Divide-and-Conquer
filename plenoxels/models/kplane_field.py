...
class KPlaneField(nn.Module):
    ...
    def get_density(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None):
        ...
        if self.spatial_distortion is not None:
            ...
            pts = pts / 2  # from [-2, 2] to [-1, 1]   # enbale it for original implementation
        else:
    ...
    def plane_visual(self, ms_grids, is_pca=False):
        data_set = self.dset
        Hres_planes = ms_grids[-1]
        planes = torch.cat([i for i in Hres_planes], dim=0)
        if is_pca:
            planes_flat = planes.flatten(2)
            planes_perm = planes_flat.permute(0,2,1)
            planes_reshape = planes_perm.reshape(-1, planes_perm.shape[-1])
            
            U, S, V = torch.pca_lowrank(planes_reshape.cpu())
            planes_red = torch.matmul(planes_reshape.cpu(), V[:, :3])
            planes_red = planes_red.reshape(*planes_perm.shape[:2], -1)
            planes_2d = planes_red.permute(0, 2, 1)
            planes_2d = planes_2d.reshape(*planes_2d.shape[:2], *planes.shape[-2:])

        else: 
            planes_avg = planes.mean(dim=1, keepdim=True)
        planes_list = [p for p in planes_avg]
        show(planes_list)

        save_path = 'plane_visual/'
        plt.savefig(f'{save_path}{data_set}.png')