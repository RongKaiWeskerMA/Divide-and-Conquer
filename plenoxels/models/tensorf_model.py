...
from plenoxels.models.density_field_tensorf import TensoRFDensityField

from plenoxels.models.tensorf_field import TensoRFField
...
from tensorf_components.components import NeRFEncoding, TensorVMEncoding 
...
class LowrankModel(nn.Module):
    def __init__(self,
        ...
        ## have to specify if it is distillation phase, treated similar to traininng phase
        self.kwargs = kwargs
        self.distil = kwargs['distil']
        self.scene = kwargs['data_dirs'][0].split('/')[-1]
        self.dset = kwargs['data_dirs'][0].split('/')[-2]
        ...
        self.field = TensoRFField(aabb,
                                  ...
                                  density_encoding=TensorVMEncoding(resolution=128, num_components=64),
                                  color_encoding=TensorVMEncoding(resolution=128, num_components=64),
                                  appearance_dim=27,
        ...
        for i in range(self.num_proposal_iterations):
            ...
            network = TensoRFDensityField(aabb, TensorVMEncoding(prop_net_args[i]['resolution'], prop_net_args[i]['num_components']))
    ...
    # @staticmethod
    def render_rgb(self, rgb: torch.Tensor, weights: torch.Tensor, bg_color: Optional[torch.Tensor]):
        comp_rgb = torch.sum(weights * rgb, dim=-2)
        accumulated_weight = torch.sum(weights, dim=-2)
        if bg_color is None:
            pass
        else:
            if self.dset == 'nerf_synthetic':
                comp_rgb = comp_rgb + (1.0 - accumulated_weight) * bg_color # enbale when working with synthetic dataset
            else:
                comp_rgb = comp_rgb + (1.0 - accumulated_weight)
        return comp_rgb
    ...
    def forward(self, rays_o, rays_d, bg_color, near_far: torch.Tensor, timestamps=None):
        ...
        ray_samples, weights_list, ray_samples_list, density_list, alpha_list = \
        ...
        #       we need to save weights at this stage and ray_samples for the distillation process
        weights, alphas = ray_samples.get_weights(density)
        ...
        if self.training or self.distil:
    ...
    def get_params(self, lr: float):
        ...
        field_params = model_params["fields"]
        encoding_params = model_params["encodings"] + [p for pnp in pn_params for p in pnp["encoding"]]
        return [
            {"params": field_params, "lr": lr},
            {"params": encoding_params, "lr": lr},
        ]
