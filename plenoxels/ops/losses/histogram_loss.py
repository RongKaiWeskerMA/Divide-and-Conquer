...
def interlevel_loss_dns(density_exp, ray_samples_exp, density_list, ray_samples_list):
    """
    This function duplicates the interlevel_loss, but it computes a loss for density
    histograms between experts' NeRF network and student proposal networks.
    """
    c = ray_samples_to_sdist(ray_samples_exp).detach()
    dns = density_exp[..., 0].detach()
    loss_interlevel = 0.0
    for ray_samples, density in zip(ray_samples_list[:-1], density_list[:-1]):
        sdist = ray_samples_to_sdist(ray_samples)
        c_ = sdist
        dns_ = density[..., 0]
        loss_interlevel += torch.mean(lossfun_outer(c, dns, c_, dns_))
    return loss_interlevel
