...
def load_data(model_type: str, data_downsample, data_dirs, validate_only: bool, render_only: bool, **kwargs):
    ...
    if model_type == "video":
        ...
        return static_trainer.load_data_distil(data_dirs, **kwargs)
...
def init_trainer(model_type: str, **kwargs):
    if model_type == "video":
        ...
        from plenoxels.runners import static_trainer_distil
        return static_trainer_distil.StaticTrainer_distil(**kwargs)
    else:
        from plenoxels.runners import static_trainer
...
def args_ovr_parse(config, over):
    overrides: List[str] = over
    overrides_dict = {ovr.split("=")[0]: ovr.split("=")[1] for ovr in overrides}

    for k, v in overrides_dict.items():
        if type(config[k]) != type(v) and config[k]:
            _type = type(config[k])
            if _type == list and type(config[k][0]) == str:
                overrides_dict[k] = [v]
            elif _type == list and type(config[k][0]) == float:
                overrides_dict[k] = [float(x) for x in re.findall(r"[-+]?\d*\.\d+|\d+", v)]
            else:
                overrides_dict[k] = _type(v)
...
def main():
    ...
    # that's derived from config - and should not be a string.
    
   
    overrides_dict = args_ovr_parse(config, args.override)
    config.update(overrides_dict)
    config['val_only'] = args.validate_only
    config["distil"] = args.distillation
    ...
    if args.log_dir is not None:
        if config['expert_idx']:
            checkpoint_path = os.path.join(args.log_dir, f"{config['expert_idx']}.pth")
        else:    
