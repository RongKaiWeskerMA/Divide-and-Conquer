...
from plenoxels.ops.losses.histogram_loss import interlevel_loss, interlevel_loss_dns
        ...
        if self.visualize:
            if self.count % 500 == 0:
                ...
                for i in range(10):  # plot 10 rays
                    ...
                    plt.savefig(f'./histogram_loss/{self.count}/batch_{i}.svg')
                    plt.close()
                    plt.cla()
                    plt.clf()
            self.count += 1
...
class DnsHistogramLoss(Regularizer):
    def __init__(self, initial_value):
        super().__init__('histogram-dns-loss', initial_value)
    def _regularize(self, model: LowrankModel, model_out, **kwargs) -> torch.Tensor:
        return interlevel_loss_dns(model_out['weights_list'], model_out['ray_samples_list'])
