...
import pickle
from plenoxels.datasets import SyntheticNerfDataset, LLFFDataset, SyntheticNerfDataset_Distil, TanksandTemple
...
class StaticTrainer(BaseTrainer):
    def __init__(self,
        ...
        self.kwargs = kwargs
        if kwargs['distil']:
            self.distil_dset = kwargs['distil_dset']
    ...
    def validate(self):
        ...
        for img_idx, data in enumerate(dataset):
            ...
            pb.update(1)
        dset = self.kwargs['data_dirs'][0].split('/')[-1]
        
        if self.kwargs['val_only']:
            if self.kwargs['expert_idx']:
                with open(f"{dset}_{self.kwargs['expert_idx']}.pkle", 'wb') as fp:
                    pickle.dump(per_scene_metrics, fp)
            else:
                with open(f"{dset}_baseline.pkle", 'wb') as fp:
                    pickle.dump(per_scene_metrics, fp)
            print("saved per-view metric")
...
def decide_dset_type(dd) -> str:
    if ("chair" in dd or "drums" in dd or "ficus" in dd or "hotdog" in dd
            ...
            or "ship" in dd or "real_scene" in dd):
        return "synthetic"
        ...
        return "llff"
    elif ("M60" in dd or "Playground" in dd or 
          "Train" in dd or "Truck" in dd):
        return "tanksandtemple" 
    else:
        raise RuntimeError(f"data_dir {dd} not recognized as LLFF or Synthetic dataset.")


def init_distil_data(data_dirs: Sequence[str], split: str, **kwargs):
    assert len(data_dirs) == 1
    data_dir = data_dirs[0]
    exp_idx = kwargs['expert_idx']
    overlap = kwargs['overlap_split']
    dset_type = decide_dset_type(data_dir)
    batch_size = int(kwargs['batch_size'])
    if dset_type == "synthetic":
        max_ts_frames = parse_optint(kwargs.get('max_ts_frames'))
        dset = SyntheticNerfDataset_Distil(
            data_dir, split=split, batch_size=batch_size, downsample=1, max_frames=max_ts_frames, exp_idx=exp_idx, overlap=overlap)
    elif dset_type == "tanksandtemple":
        max_ts_frames = parse_optint(kwargs.get('max_ts_frames'))
        dset = TanksandTemple(
            data_dir, split=split, batch_size=batch_size, downsample=1, max_frames=max_ts_frames, exp_idx=exp_idx)
...
def init_tr_data(data_downsample: float, data_dirs: Sequence[str], **kwargs):
    ...
    if dset_type == "synthetic":
        ...
        dset = SyntheticNerfDataset(
            ...
            max_frames=max_tr_frames, batch_size=batch_size, exp_idx = exp_idx, overlap=overlap)
    elif dset_type == "tanksandtemple":
        max_tr_frames = parse_optint(kwargs.get('max_tr_frames'))
        dset = TanksandTemple(
            data_dir, split='train', downsample=data_downsample,
            max_frames=max_tr_frames, batch_size=batch_size, exp_idx = exp_idx)
    elif dset_type == "llff":
        hold_every = parse_optint(kwargs.get('hold_every'))
        dset = LLFFDataset(
...
def init_ts_data(data_dirs: Sequence[str], split: str, **kwargs):
    ...
    if dset_type == "synthetic":
        max_ts_frames = parse_optint(kwargs.get('max_ts_frames'))
        dset = SyntheticNerfDataset(
            data_dir, split=split, downsample=1, max_frames=max_ts_frames, exp_idx=exp_idx, overlap=overlap)
    elif dset_type == "tanksandtemple":
        max_ts_frames = parse_optint(kwargs.get('max_ts_frames'))
        dset = TanksandTemple(
            data_dir, split=split, downsample=1, max_frames=max_ts_frames, exp_idx=exp_idx)
    elif dset_type == "llff":
        hold_every = parse_optint(kwargs.get('hold_every'))
        dset = LLFFDataset(
            data_dir, split=split, downsample=4, hold_every=hold_every,
...
# We save all the prediction from experts to form 
# the ground-truth for the distillation process
def load_data_distil( data_dirs, **kwargs):
    od: Dict[str, Any] = {}
    od.update(tr_loader=None, tr_dset=None)
    od.update(init_distil_data(data_dirs, 'train', **kwargs))
    test_split = 'test'
    od.update(init_ts_data(data_dirs, split=test_split, **kwargs))
    return od
